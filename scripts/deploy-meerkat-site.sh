...
usage() {
	cat <<'EOF'
Usage:
  deploy-meerkat-site.sh [options]

Deploy the static meerkat.tnisoft.ro marketing site to vps1.

By default this script runs in dry-run mode. Pass --apply to upload and switch
the live release.

Options:
  --source PATH       Static site folder. Defaults to the meerkat-site/ folder
                      inside the meerkat repo.
  --host HOST         SSH host. Defaults to vps1.
  --remote PATH       Remote app root. Defaults to /srv/tnisoft/meerkat.
...
Examples:
  deploy-meerkat-site.sh
  deploy-meerkat-site.sh --apply
EOF
}
...
SOURCE="${MEERKAT_SITE_SOURCE:-$HOME/projects/meerkat/meerkat-site}"
HOST="${MEERKAT_SITE_HOST:-vps1}"
REMOTE_ROOT="${MEERKAT_SITE_REMOTE_ROOT:-/srv/tnisoft/meerkat}"
...
for required_path in "index.html" "styles.css" "privacy.html" "terms.html"; do
...
info "Verifying release on host..."
ssh "$HOST" "test -f '$REMOTE_CURRENT/index.html' && echo 'public -> '\$(readlink -f '$REMOTE_CURRENT')"

info "Checking public endpoint (soft) ..."
if curl -fsS --max-time 8 "https://meerkat.tnisoft.ro/" >/dev/null 2>&1; then
	info "Live: https://meerkat.tnisoft.ro/ responded."
else
	info "Note: https://meerkat.tnisoft.ro/ not reachable yet (Caddy vhost / TLS may still be needed)."
fi

info "Meerkat site deployed: $RELEASE"
