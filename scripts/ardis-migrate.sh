...
# Load forge config (robust lookup)
if [[ -f "$SCRIPT_DIR/forge.sh" ]]; then
	...
	echo "forge.sh not found (checked: $SCRIPT_DIR/forge.sh, $HOME/mac-forge/scripts/forge.sh, $HOME/mac-forge/forge.sh)." >&2
	exit 1
fi
...
choose_database() {
	...
	echo "Querying databases from SQL on ${FORGE_SQL_HOST},${FORGE_SQL_PORT} (container: '$FORGE_SQL_DOCKER_CONTAINER')..." >&2
